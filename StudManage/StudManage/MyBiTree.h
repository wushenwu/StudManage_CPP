...
private:
    ...
    CTreeNode<T> *m_pNext;     //针对可能的相同数据的链表处理
    CTreeNode<T> *m_pPre;       //针对可能的相同数据的链表处理
...
public:
    ...
    */
    ...
    /*
    Function:查找满足给定Data和文件中偏移nInfoOffset的结点，如果存在则返回相应的结点信息，
             否则NULL。
...
private:
    ...
    */
    ...
    /*
    Function: 对相同元素的链表的处理, 删除单孩子结点，双孩子结点
    */
...
CMyBiTree<T>::Insert(T Data, unsigned long nInfoOffset)
{
    ...
    */
    ...
    while (true)
    {
        ...
        else if (0 > EleCompare(pNewNode->m_Data, pCurNode->m_Data)) //查找左子树
        {
...
CMyBiTree<T>::Remove(CTreeNode<T> *pDelNode)
{
    ...
    /*
    现在因为加入了链表，对相同Key的进行了处理，所以，pDelNode也可能是链表中的结点：
    1)仍然按原有的思路进行分类：叶子结点，仅一个孩子的结点，有两个孩子的结点
    2)对这些分类加入特殊的处理：
        a)叶子结点：链表中的结点会出现在这里，它们除了无孩子外，还没有父结点。
                    特殊的，链表的头结点，可能有父结点存在
        b)单孩子
        c)双孩子的情况
    */
    ...
    else    
    {
        ...
        */
        ...
        //因为加入了对相同元素的链表处理，所以，首先该结点是链表的首结点
        if (pDelNode->m_pNext != NULL)
        {
