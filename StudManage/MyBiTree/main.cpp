...
public:
    unsigned int m_nSelectiveID;    //选课ID，由学号ID和课程ID组成
};
...
template<typename T>
class CTreeNode_NON //不带链表的处理
{
protected:
    T m_Data;

    //ID相关(或者选课中StudID + CourseID
...
{
protected:
    CTreeNode<T> *m_pNext; //针对同名的链表处理
};

template<typename T>
class CMyBiTree_NON     //不带链表的tree
{
private:    //不能被继承
...
private:
    ...
    //二叉树相关
    //或许可以两两合而为一
    CMyBiTree_NON<CStudent> m_studIDTree;   //不带链表的二叉树
    CMyBiTree<CStudent> m_studNameTree;     //带有链表的二叉树
